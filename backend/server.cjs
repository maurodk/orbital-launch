...
app.get("/api/diretoria", verifyToken, async (req, res) => {
  try {
    // Busca unidades e pagamentos (service role supabase)
    ...
    // Helpers para normalizar campos numéricos/strings
    const toNumber = (v) => {
      if (v == null) return 0;
      const n = Number(v);
    ...
    // Contagens por tipologia / imobiliaria / corretor (apenas unidades com situação reservada)
    ...
    // Totais a partir da tabela pagamentos (soma de pagamentos não-cancelados)
    ...
    // Quantidade de unidades bloqueadas / disponíveis
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const unidadesToInsert = dataLines.filter((cols) => {
        return (
          Array.isArray(cols) &&
          cols.length >= 3 &&
          cols[0] &&
