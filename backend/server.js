...
app.post("/api/update-pix-data", verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Atualiza as colunas N (identificador), O (payloadEmv), P (Valor), Q (Status Pagamento) e R (Timestamp PIX)
    await sheets.spreadsheets.values.update({
      spreadsheetId: SPREADSHEET_ID_IMPLANTACAO,
      range: `'${sheetTitle}'!N${rowIndex}:R${rowIndex}`,
      valueInputOption: "USER_ENTERED",
    ...
    console.log(`[UPDATE-PIX-DATA] ✓ PIX atualizado com sucesso na planilha`);
...
// =================================================================
// JOB: CANCELAMENTO AUTOMÁTICO DE PIX EXPIRADOS (60 MINUTOS)
// =================================================================
          ...
          if (statusPagamento === "PENDENTE") {
            ...
            if (pixGeradoEntry && pixGeradoEntry[0]) {
              ...
              if (diffMinutes >= TIMEOUT_MINUTES) {
                ...
                // Colunas: F, G, H, I, J, K (status), L (preserva), M (preserva), N, O, P, Q, R
                await sheets.spreadsheets.values.batchUpdate({
                  spreadsheetId: SPREADSHEET_ID_IMPLANTACAO,
                  resource: {
                    data: [
                      ...
                      {
                        range: `'${implantacao}'!N${rowIndex}:Q${rowIndex}`, // PIX data (sem coluna R)
                        values: [["", "", "", ""]],
                      },
                    ],
                    valueInputOption: "USER_ENTERED",
